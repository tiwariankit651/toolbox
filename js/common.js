...
// Favorites feature
function toggleFav(toolName,btn){var favs=JSON.parse(localStorage.getItem('favTools')||'[]');var idx=favs.indexOf(toolName);if(idx>-1){favs.splice(idx,1);btn.textContent='🤍'}else{favs.push(toolName);btn.textContent='❤️'}localStorage.setItem('favTools',JSON.stringify(favs));refreshFavSection()}
